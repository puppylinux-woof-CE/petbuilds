...
case "$opt" in
    ...
    -o) # open: mangle output of rar cmd for xarchive 
		...
		# unrar 4-
		unrar_ver=$($RAR_PROG | (read; read unrar version etc; echo $version))
		if [ "$unrar_ver" ] ; then
			if vercmp $unrar_ver lt 5 ; then
		...
		#----------- ---------  -------- ----- ---------- -----  --------  ----
		#    ..A....   1616320   1616320 100%  2016-01-12 10:08  B1ECD309  DSCF1842.JPG
		#    ..A....   1616324   1616324 100%  2015-11-30 17:21  F1FB13F7  DSCF1843.JPG
		#----------- ---------  -------- ----- ---------- -----  --------  ----
		#              3232644   3232644 100%                              2

		# Attributes      Size    Packed Ratio    Date    Time   Checksum  Name
		#----------- ---------  -------- ----- ---------- -----  --------  ----
		#*   ..A....      1397       384  27%  2019-09-26 11:53  7AB4CBxx  Pathto/File

		$RAR_PROG $OPEN_OPTS "$archive" | \
		...
		do
			...
			printf "%s;%s;%s;%s;%s;%s;%s;-\n" \
				"$NAME" "$SIZE" "$attr" "${UID}" "${UID}" "$DATE" "$TIME"
		done
		exit
        ;;
